...
#pragma unroll 32
      for (int k = 0; k < K; k += K_STEP) {
        _tile_stream_loadd(6, pB, MIN_STRIDE); pB += TILE_SIZE; // tileload B0
        amx_tile_load_L1A(4, A, i, k, lda);                     // tileload A0
        _tile_stream_loadd(7, pB, MIN_STRIDE); pB += TILE_SIZE; // tileload B1
        amx_tile_load_L1A(5, A, i + MAX_ROWS, k, lda);          // tileload A1
        _tile_dpbssd(0, 4, 6); // tdp A0, B0
        _tile_dpbssd(2, 5, 6); // tdp A1, B0
        _tile_dpbssd(1, 4, 7); // tdp A0, B1
...
#pragma unroll 32
      for (int k = 0; k < K; k += K_STEP) {
        _tile_stream_loadd(6, pB, MIN_STRIDE); pB += TILE_SIZE; // tileload B0
        _tile_loadd(4, pA, MIN_STRIDE); pA += TILE_SIZE;        // tileload A0
        _tile_stream_loadd(7, pB, MIN_STRIDE); pB += TILE_SIZE; // tileload B1
        _tile_loadd(5, pA, MIN_STRIDE); pA += TILE_SIZE;        // tileload A1
        _tile_dpbssd(0, 4, 6); // tdp A0, B0
        _tile_dpbssd(2, 5, 6); // tdp A1, B0
        _tile_dpbssd(1, 4, 7); // tdp A0, B1
        _tile_dpbssd(3, 5, 7); // tdp A1, B1
...
#pragma unroll 32
      for (int k = 0; k < K; k += K_STEP) {
        ...
        _tile_dpbssd(3, 5, 7); // tdp A1, B1
        // prefetch A
        _mm_prefetch(pA_pf, _MM_HINT_T1); pA_pf += CACHELINE;
        _mm_prefetch(pA_pf, _MM_HINT_T1); pA_pf += CACHELINE;
      }
      amx_tile_store_L1C(0, C, i, j, ldc);
      amx_tile_store_L1C(1, C, i, j + MAX_ROWS, ldc);
...
void amx_gemm_i8i8i32_l0_tiling_prefetchAC(GEMM_PARAMS) {
  ...
  for (int i = 0; i < M; i += M_STEP) {
    ...
    for (int j = 0; j < N; j += N_STEP) {
      ...
      int8_t *pA = A + i * K; // packed A block address
      int8_t *pC_pf = (int8_t *)((j + N_STEP <= N)? &C[OFFSET2D(i, j + N_STEP, ldc)]
                                                  : &C[OFFSET2D(i + M_STEP, 0, ldc)]); // prefetch address for C
      int pfC_cnt = 0;
...
#pragma unroll 32
      for (int k = 0; k < K; k += K_STEP) {
        ...
        if (likely(pB_pf < B + 2 * N * K)) {
          _mm_prefetch(pB_pf, _MM_HINT_T2); pB_pf += CACHELINE;
          _mm_prefetch(pB_pf, _MM_HINT_T2); pB_pf += CACHELINE;
        }
        // prefetch C
        if (likely(pfC_cnt < 4096 / CACHELINE)) { //  prefetch C
...
extern int num_core, core_m, core_n;
void amx_gemm_i8i8i32_l2_tiling_omp(GEMM_PARAMS) {
  assert(num_core == core_m * core_n);
  ...
  #pragma omp parallel num_threads(num_core)
  {
    ...
    //         MIN(M - tm, TM), MIN(N - tn, TN), MIN(K - tk, TK),
    //         lda, ldb, ldc);
    //     }
    //   }
    // }

    int m_task_sz = MAX(M / core_m, TM);
    int n_task_sz = MAX(N / core_n, TN);
    ...
    int m_task_sz1 = MIN(M - m_start, m_task_sz);
    for(int tn = n_start; tn < N; tn += TN) {
      for(int tk = 0; tk < K; tk += TK) {
        amx_gemm_i8i8i32_l0_tiling_prefetchAC(
...
void amx_packBtile_i8i8i32(int8_t *pB0, int8_t *pB1, int ldb) {
  ...
  for (int k = 0; k < MAX_COLS; k++) {
    for (int n = 0; n < MAX_ROWS; n++) {
      pB1[OFFSET3D(k / kpack, n, k % kpack, MAX_ROWS, kpack)] = pB0[OFFSET2D(k, n, ldb)];
    }
  }
}
...
void amx_packA_i8i8i32(int8_t *__restrict__ A, int8_t *__restrict__ A_packed,
  ...
  int m_task_sz = MAX(M / core_m, TM);
  for (int tm = 0; tm < M; tm += m_task_sz) {
    for (int tk = 0; tk < K; tk += TK) {
      int8_t *pA0 = &A[OFFSET2D(tm, tk, M)];

      for (int m = 0; m < MIN(m_task_sz, M - tm); m += M_STEP) {
        for (int k = 0; k < MIN(TK, K - tk); k += K_STEP) {
          // pack 2 tiles of A
          amx_packAtile_i8i8i32(&pA0[OFFSET2D(m, k, K)], pA1, K);
          pA1 += TILE_SIZE;
